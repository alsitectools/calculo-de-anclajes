...
export class DiagramMuro1Cara {
  ...
  render() {
    ...
    const leftPressureW = Math.min(100, Math.max(26, (PresionMax / 25) * 60));
    ...
    for (let i = 0; i < numArrowsLeft; i++) {
      ...
      arrowsLeftSvg += `<line x1="${leftWallX - curW}" y1="${arrowY}" x2="${leftWallX - 3}" y2="${arrowY}" stroke="#38bdf8" stroke-width="1.6" marker-end="url(#arrowM1C)" />\n`;
    }

    const cotaXLeft = Math.max(22, leftWallX - leftPressureW - 25);
